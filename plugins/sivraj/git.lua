...
local function checkout_commands(path, branch, ref, upstream)
  ref = ref or "HEAD"
  local commands = { { "git", "-C", path, "clean", "-fd" } }
  if branch and branch ~= "" then
    commands[#commands + 1] = { "git", "-C", path, "checkout", "-f", "-B", branch, ref }
    if upstream and upstream ~= "" then
      commands[#commands + 1] = { "git", "-C", path, "branch", "--set-upstream-to=" .. upstream, branch }
    end
  else
    commands[#commands + 1] = { "git", "-C", path, "checkout", "-f", "--detach", ref }
  end
  commands[#commands + 1] = { "git", "-C", path, "reset", "--hard", ref }
  commands[#commands + 1] = { "git", "-C", path, "clean", "-fd" }
  return commands
end
...
function M.sync_remote_repo(repo, yielding)
  ...
  for _, wt in ipairs(mapped) do
    ...
    if wt.path == repo.cache_path then
      for _, command in ipairs(checkout_commands(wt.path, wt.branch_name, ref, upstreams[wt.branch_name])) do
    ...
    elseif existing[wt.path] then
      ...
      if wt.branch_name and wt.branch_name ~= "" then
        table.move({ "-B", wt.branch_name, wt.path, ref }, 1, 4, #args + 1, args)
      else
        table.move({ "--detach", wt.path, ref }, 1, 3, #args + 1, args)
      end
      output, code = run(repo.cache_path, args, yielding)
      if code ~= 0 then return false, output end
      if wt.branch_name and upstreams[wt.branch_name] and upstreams[wt.branch_name] ~= "" then
        output, code = run(wt.path, { "branch", "--set-upstream-to=" .. upstreams[wt.branch_name], wt.branch_name }, yielding)
        if code ~= 0 then return false, output end
      end
    end
  end
